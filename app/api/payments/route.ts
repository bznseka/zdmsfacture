...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      rows.map((row) => shapePayment(row, row.invoice?.invoiceNumber, row.invoice?.client?.name))
    );
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(shapePayment(row, invoice.invoiceNumber, invoice.client?.name), {
      status: 201,
    });
  } catch (error) {
    return apiErrorResponse(error);
